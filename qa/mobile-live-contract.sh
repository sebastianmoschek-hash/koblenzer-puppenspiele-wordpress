...
grep -q 'addJavascriptInterface(NativeAiBridge(), "KPLocalLive")' "$MAIN"
grep -q 'private fun dispatchLocalLive' "$MAIN"
grep -q "CustomEvent('kp:local-live'" "$MAIN"
grep -q 'requestId = cleanId' "$MAIN"
grep -q 'WebView.setWebContentsDebuggingEnabled(true)' "$MAIN"
...
grep -q 'MODEL_SHA256' "$LOCAL"
grep -q 'header("Range", "bytes=\$existingBytes-")' "$LOCAL"
grep -q 'Der nächste Versuch wird an dieser Stelle fortgesetzt' "$LOCAL"
