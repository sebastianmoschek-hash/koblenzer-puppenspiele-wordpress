...
try {
  for (const viewport of viewports) {
    ...
    const result = await page.evaluate(async () => {
      ...
      unbindGestures();
      unbind();
      return { before, changed, restored, renderedText, selected, gestureMoved, styled, sectionSlice, navigationSlice, designSlice, duplicateDelete, imageSlice, persistenceRestored, aiContractReady, schemaMigration, previewTransactions, undoRestored: restored === before, schema: v2.SCHEMA_VERSION };
    });
    ...
    if (!result.gestureMoved || !result.imageSlice || !result.sectionSlice || !result.navigationSlice || !result.designSlice || !result.duplicateDelete || !result.persistenceRestored || !result.aiContractReady || !result.schemaMigration || !result.previewTransactions) failures.push(`${viewport.name}: V2-Slice unvollständig`);
