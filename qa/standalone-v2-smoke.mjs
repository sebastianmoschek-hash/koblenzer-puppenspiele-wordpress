...
try {
  for (const viewport of viewports) {
    ...
    await page.locator('#kpV2AISheet [data-close]').focus();
    await page.keyboard.press('Tab');
    result.accessibilityUI = await page.evaluate(() => { const dialog = document.querySelector('#kpV2AISheet'), visibleControls = [...document.querySelectorAll('#editor button,#kpV2Toolbar button,#kpV2AISheet button')].filter(node => node.getClientRects().length); return dialog?.getAttribute('role') === 'dialog' && dialog?.getAttribute('aria-modal') === 'true' && dialog.contains(document.activeElement) && document.querySelector('#kpV2Toolbar')?.getAttribute('role') === 'toolbar' && document.querySelector('#status')?.getAttribute('aria-live') === 'polite' && visibleControls.every(node => (node.getAttribute('aria-label') || node.textContent).trim().length > 0 && node.getBoundingClientRect().height >= 44); });
