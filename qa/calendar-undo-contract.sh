...
for file in "$HISTORY" "$CAL" "$GUARD" "$SERVER" "$IMPORTER"; do
...
if grep -Eq 'wp_delete_post|wp_delete_attachment' "$CAL" "$GUARD"; then
  fail 'calendar Undo must not permanently delete WordPress content'
fi

echo 'PASS: calendar connection/sync/draft/publish are reversible, conflict-safe and Google remains read-only.'
