...
(() => {
  ...
  function createActions(store) {
    ...
    const api = {
      ...
      updateNavigationItem: (itemId, changes = {}) => store.transact('Menüpunkt bearbeiten', state => { const item = state.document.navigation.items.find(entry => entry.id === itemId); if (item) { if (changes.label != null) item.label = String(changes.label); if (changes.href != null) item.href = String(changes.href); } }),
      ...
      createNavigationSection: (label = 'Neuer Abschnitt', pageId = 'home') => store.transact('Menüpunkt und Abschnitt hinzufügen', state => { const page = state.document.pages.find(item => item.id === pageId) || state.document.pages[0]; if (!page) return; const sectionId = id('section'), itemId = id('nav'); page.sections.push({ id: sectionId, order: page.sections.length, design: { className: 'dark editable', type: 'content', layout: 'normal' }, elements: [{ id: id('el'), type: 'heading', order: 0, content: { text: String(label) }, styles: {}, transform: { x: 0, y: 0, scale: 1, rotation: 0 }, source: { tag: 'h2', className: '' } }] }); state.document.navigation.items.push({ id: itemId, label: String(label), href: '#' + sectionId, order: state.document.navigation.items.length }); return { sectionId, itemId }; }),
      ...
      deleteNavigationTarget: (itemId, deleteTarget = false) => store.transact(deleteTarget ? 'Menüpunkt und Abschnitt löschen' : 'Menüpunkt löschen', state => { const item = state.document.navigation.items.find(entry => entry.id === itemId), targetId = item?.href?.startsWith('#') ? item.href.slice(1) : ''; state.document.navigation.items = state.document.navigation.items.filter(entry => entry.id !== itemId); state.document.navigation.items.forEach((entry, order) => { entry.order = order; }); if (deleteTarget && targetId && targetId !== 'home') for (const page of state.document.pages) page.sections = page.sections.filter(section => section.id !== targetId); }),
