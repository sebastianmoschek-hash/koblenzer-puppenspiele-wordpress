(()=>{
 ...
 function apply(v=layout()){v=normalize(v);menu.removeAttribute('style');if(v.size)menu.style.setProperty('--menu-size',v.size+'px');if(Number.isFinite(v.left)){const size=v.size||menu.getBoundingClientRect().width||56;menu.style.left=Math.min(innerWidth-size-8,v.left)+'px';menu.style.right='auto'}if(Number.isFinite(v.top)){const size=v.size||menu.getBoundingClientRect().height||56;menu.style.top=Math.min(innerHeight-size-8,v.top)+'px'}localStorage.setItem(KEY,JSON.stringify(v))}
