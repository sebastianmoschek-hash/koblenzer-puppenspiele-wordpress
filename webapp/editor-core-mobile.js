...
function beforeEdit(){clearTimeout(timer);if(!current)current=capture()}
...
edit.addEventListener('click',()=>{editable(true);current=capture();past=[];future=[];sync();say('Editor aktiv')});
...
if(undo)undo.addEventListener('click',()=>{clearTimeout(timer);const now=capture();if(now!==current)commit(now);if(!past.length){sync();return}future.push(current);trim();const previous=past.pop();restore(previous);say('Rückgängig')});
...
window.__kpUndoCheckpoint=()=>{clearTimeout(timer);commit(capture());const before=capture();return()=>{if(restoring)return;const now=capture();if(now===before){sync();return}past.push(before);trim();current=now;future=[];sync();say('Nicht gespeichert')}};
