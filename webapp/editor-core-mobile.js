...
function flushPending(){clearTimeout(timer);const now=snapshot();if(now!==current)commit(now)}
...
if(close)close.addEventListener('click',()=>{flushPending();clearSelection();editable(false)});
if(add)add.addEventListener('click',()=>{flushPending();const before=snapshot();const s=document.createElement('section');s.innerHTML='<div class="wrap"><p class="eyebrow">Neuer Abschnitt</p><h2>Neue Überschrift</h2><p>Hier Text eingeben.</p></div>';q('main').appendChild(s);current=before;commit(snapshot());editable(true)});
if(undo)undo.addEventListener('click',()=>{flushPending();if(!past.length){sync();return}future.push(current);trim();restore(past.pop());say('Rückgängig')});
if(redo)redo.addEventListener('click',()=>{flushPending();if(!future.length){sync();return}past.push(current);trim();restore(future.pop());say('Wiederholt')});
async function cloud(){flushPending();try{say('Speichere …');const r=await fetch('./api/editor-state.php',{method:'PUT',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({version:3,savedAt:new Date().toISOString(),html:capture(),layout:{}}),cache:'no-store'});if(!r.ok)throw new Error('Server '+r.status);try{localStorage.setItem('kp-webapp-content-v2',capture())}catch(e){}say('Dauerhaft auf dem Webspace gespeichert ✓')}catch(e){say('Speichern fehlgeschlagen: '+e.message)}}
if(save)save.addEventListener('click',cloud);
if(localSave)localSave.addEventListener('click',()=>{flushPending();try{localStorage.setItem('kp-webapp-content-v2',capture());say('Auf diesem Gerät gespeichert ✓')}catch(e){say('Gerätespeicher fehlgeschlagen')}});
window.__kpUndoCheckpoint=()=>{flushPending();const before=snapshot();return()=>{if(restoring)return;const now=snapshot();if(now===before){sync();return}past.push(before);trim();current=now;future=[];sync();say('Nicht gespeichert')}};
