...
function bindInputs(){qa('main [contenteditable=true]').forEach(el=>{if(el.dataset.kpHistoryBound)return;el.dataset.kpHistoryBound='1';el.addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(()=>record(snapshot()),400)});el.addEventListener('blur',flush)})}
...
function doUndo(){clearTimeout(timer);const now=snapshot();if(now!==current)record(now);if(!past.length){sync();return}const previous=past.pop();future.push(current);trim();restore(previous);say('Rückgängig')}
function doRedo(){clearTimeout(timer);if(!future.length){sync();return}const next=future.pop();past.push(current);trim();restore(next);say('Wiederholt')}
window.__kpHistory={undo:doUndo,redo:doRedo,checkpoint(){flush();const before=current||snapshot();let done=false;return()=>{if(done||restoring)return;done=true;const now=snapshot();if(now===before){sync();return}past.push(before);trim();current=now;future=[];sync();say('Nicht gespeichert')}},sync};
...
function ownButtons(){if(undo){undo.onclick=null;undo.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();doUndo()},true)}if(redo){redo.onclick=null;redo.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();doRedo()},true)}}
