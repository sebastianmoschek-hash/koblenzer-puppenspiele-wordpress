...
if($method==='GET'){$data=readState($file);if(!$data){echo json_encode(['ok'=>true,'exists'=>false,'version'=>2]);exit;}$data['ok']=true;$data['exists']=true;echo json_encode($data,JSON_UNESCAPED_UNICODE);exit;}
...
$previous=readState($file);if($previous&&is_string($previous['html']??null)){$prevTime=$previous['savedAt']??gmdate('c');$prevId=gmdate('Ymd-His',strtotime((string)$prevTime)?:time()).'-'.substr(hash('sha256',$previous['html']),0,8);$prevPath=$historyDir.'/'.$prevId.'.json';if(!is_file($prevPath))file_put_contents($prevPath,json_encode($previous,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),LOCK_EX);}
$data=['version'=>2,'savedAt'=>gmdate('c'),'html'=>$input['html']];$tmp=$file.'.tmp';if(file_put_contents($tmp,json_encode($data,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),LOCK_EX)===false||!rename($tmp,$file)){http_response_code(500);echo json_encode(['error'=>'save failed']);exit;}
$currentId=gmdate('Ymd-His').'-'.substr(hash('sha256',$data['html']),0,8);file_put_contents($historyDir.'/'.$currentId.'.json',json_encode($data,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),LOCK_EX);
$files=glob($historyDir.'/*.json')?:[];usort($files,fn($a,$b)=>filemtime($b)<=>filemtime($a));foreach(array_slice($files,50) as $old)@unlink($old);
echo json_encode(['ok'=>true,'exists'=>true,'savedAt'=>$data['savedAt'],'versionId'=>$currentId]);
