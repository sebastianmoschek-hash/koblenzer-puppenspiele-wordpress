...
add_action( 'wp_footer', static function () {
    ...
    (()=>{
      ...
      q('.kp-ai-route-select',sheet)?.addEventListener('change',async(e)=>{const select=e.currentTarget;select.disabled=true;try{const d=await api('kp_ai_route_save',{route:select.value});cfg.route=d.route||select.value;setStatus(d.message||'KI-Modus gespeichert ✓')}catch(err){setStatus(err.message||'KI-Modus konnte nicht gespeichert werden.')}finally{select.disabled=false}});
