...
add_action( 'wp_footer', static function () {
    ...
    (()=>{
      ...
      function renderImages(){
        qa('.kp-header-stage img,.kp-header-photo img').forEach(img=>{
      ...
      function discard(){current={...saved};dirty=false;history.length=0;redo.length=0;renderImages();renderPreview()}
      ...
      function syncPanel(){if(q('.kp-oa-header-preview'))renderPreview()}
      wrapMedia();syncPanel();
      new MutationObserver(()=>{wrapMedia();syncPanel()}).observe(document.documentElement,{childList:true,subtree:true});
    })();
    </script>
    <?php
}, 2175 );
