...
add_action( 'wp_footer', static function () {
    ...
    (()=>{
      ...
      function apply(state){current={id:Number(state?.id)||0,url:String(state?.url||'')};const images=qa('.kp-header-stage img,.kp-header-photo img');images.forEach(img=>{if(current.url){img.src=current.url;img.removeAttribute('srcset');img.removeAttribute('sizes')}});const preview=q('.kp-oa-header-preview');if(preview&&current.url)preview.innerHTML=`<img src="${current.url.replace(/"/g,'&quot;')}" alt="Header-Vorschau">`;dirty=true;q('.kp-fe2-save')?.classList.add('is-dirty');return true}
      ...
      const runtime={flush,isDirty:()=>dirty,undo,redo:redoStep,clearRedo,counts:()=>({undo:history.length,redo:redo.length})};window.KPHeaderImageDraftRuntime=runtime;
