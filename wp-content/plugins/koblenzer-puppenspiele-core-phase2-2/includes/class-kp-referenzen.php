...
class KP_Referenzen {
    ...
    public static function ensure_references_page() {
        ...
        if ( $existing ) {
            if ( trim( (string) $existing->post_content ) === '' ) {
                wp_update_post( [ 'ID' => $existing->ID, 'post_content' => '[kp_referenzen]' ] );
            }
            return (int) $existing->ID;
        }
        $page_id = wp_insert_post( [
            'post_type' => 'page', 'post_status' => 'publish', 'post_title' => 'Referenzen',
            'post_name' => 'referenzen', 'post_content' => '[kp_referenzen]', 'comment_status' => 'closed',
        ] );
    ...
    public static function register_post_type() {
        register_post_type( self::POST_TYPE, [
            'labels' => [
                'name' => 'Referenzen','singular_name' => 'Referenz','add_new' => 'Referenz hinzufügen',
                'add_new_item' => 'Referenz hinzufügen','edit_item' => 'Referenz bearbeiten','new_item' => 'Neue Referenz',
                'view_item' => 'Referenz ansehen','search_items' => 'Referenzen suchen','not_found' => 'Keine Referenzen gefunden','menu_name' => 'Referenzen',
            ],
            'public' => false,'show_ui' => true,'show_in_menu' => false,'supports' => [ 'title', 'thumbnail', 'page-attributes' ],'show_in_rest' => true,
        ] );
    }

    public static function add_meta_boxes() {
        add_meta_box( 'kp_referenz_details', 'Referenz-Details', [ __CLASS__, 'meta_box' ], self::POST_TYPE, 'normal', 'high' );
    }
    ...
    public static function admin_menu() {
        add_submenu_page( 'kp-puppenspiele','Referenzen','Referenzen','edit_posts','edit.php?post_type=' . self::POST_TYPE );
        add_submenu_page( 'kp-puppenspiele','Referenzen importieren','Referenzen importieren','manage_options','kp-referenzen-import',[ __CLASS__, 'import_page' ] );
    }

    public static function import_page() {
        if ( ! current_user_can( 'manage_options' ) ) return;
        ?>
        <div class="wrap"><h1>Aktuelle Referenzen übernehmen</h1>
        ...
        <input type="hidden" name="action" value="kp_import_referenzen"><?php wp_nonce_field( 'kp_import_referenzen' ); ?>
        <?php submit_button( '25 aktuelle Referenzen jetzt importieren', 'primary' ); ?></form></div>
        <?php
    }
    ...
    public static function handle_import() {
        ...
        foreach ( $items as $item ) {
            ...
            $existing = get_posts( [ 'post_type'=>self::POST_TYPE,'post_status'=>'any','meta_key'=>self::META_LEGACY_KEY,'meta_value'=>$legacy_key,'fields'=>'ids','posts_per_page'=>1 ] );
            if ( $existing ) { $skipped++; continue; }
            $post_id = wp_insert_post( [ 'post_type'=>self::POST_TYPE,'post_status'=>'publish','post_title'=>$item['title'] ] );
            ...
            if ( ! empty( $item['bundled_image'] ) && file_exists( $img ) ) {
                ...
                if ( empty( $upload['error'] ) ) {
                    ...
                    $attachment_id = wp_insert_attachment( [ 'post_mime_type'=>$filetype['type'],'post_title'=>sanitize_text_field($item['title']),'post_status'=>'inherit' ], $upload['file'], $post_id );
        ...
        wp_safe_redirect( add_query_arg( [ 'page'=>'kp-puppenspiele','kp_ref_created'=>$created,'kp_ref_skipped'=>$skipped ], admin_url( 'admin.php' ) ) );
        exit;
    }

    public static function shortcode( $atts ) {
        $q = new WP_Query( [ 'post_type'=>self::POST_TYPE,'post_status'=>'publish','posts_per_page'=>-1,'orderby'=>[ 'menu_order'=>'ASC','title'=>'ASC' ],'order'=>'ASC' ] );
        if ( ! $q->have_posts() ) return '<p class="kp-referenzen-empty">Noch keine Referenzen angelegt.</p>';
        ob_start(); ?>
        <section class="kp-referenzen"><div class="kp-referenzen-grid">
        <?php while ( $q->have_posts() ) : $q->the_post();
            ...
            <article class="kp-referenz-card">
              <?php if ( $url ) : ?><a class="kp-referenz-link" href="<?php echo esc_url( $url ); ?>" target="_blank" rel="noopener noreferrer"><?php endif; ?>
                <div class="kp-referenz-image"><?php if ( $img ) : ?><img src="<?php echo esc_url( $img ); ?>" alt="<?php echo esc_attr( get_the_title() ); ?>" loading="lazy"><?php endif; ?></div>
                <div class="kp-referenz-copy"><h3><?php the_title(); ?></h3>
